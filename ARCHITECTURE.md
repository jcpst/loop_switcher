...
│                     ModeController                              │
│                                                                 │
│  - detectSwitchPatterns()    : Interpret user input            │
│  - updateStateMachine()      : Handle timing & transitions     │
│  - handleSingleSwitchPress() : Mode-specific switch actions    │
│                                                                 │
└──────────────────────┬──────────────────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────────────────┐
...
│                    StateManager                                 │
│                                                                 │
│  State Variables:                EEPROM Operations:            │
│  - currentMode                   - savePreset()                │
│  - currentBank                   - loadPreset()                │
│  - loopStates[4]                 - initialize()                │
│  - activePreset                                                 │
│  - midiChannel                   Data Accessors:               │
│  - displayState                  - getDisplayValue()           │
│  - editModeLoopStates[4]         - getDisplayLoops()           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
│                                                                 │
│  loop() {                                                       │
│    1. switches.readAndDebounce()     ← Read all 4 switches     │
│    2. modeController.detectPatterns() ← Interpret combos        │
│    3. modeController.updateState()    ← Handle transitions     │
│    4. relays.update()                 ← Apply loop states      │
│    5. leds.update()                   ← Update status LEDs     │
│    6. display.update()                ← Update 7-segment       │
│  }                                                              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
### Switch Press → MIDI Output

```
User Presses SW1
       │
       ▼
SwitchHandler.readAndDebounce()
       │ (detects LOW, starts timer)
       ▼
ModeController.detectSwitchPatterns()
       │ (check if recent press)
       ▼
ModeController.handleSingleSwitchPress(0)
       │
       ├─ MANUAL_MODE ──→ Toggle loopStates[0]
       │
       ├─ BANK_MODE ────→ Calculate PC = (bank × 4) + 0 + 1
       │                  │
       │                  ├─→ sendMIDIProgramChange(PC, channel)
       │                  │       │
       │                  │       └→ Serial.write(0xC0 | channel)
       │                  │          Serial.write(PC - 1)
       │                  │
       │                  └─→ loadPreset(PC) from EEPROM
       │                          │
       │                          └─→ Update loopStates[4]
       │
       └─ EDIT_MODE ─────→ Toggle editModeLoopStates[0]
...
Main Loop Updates
       ...
       │
       ├─→ leds.update(loopStates, mode, preset)
       │       │
       │       └─→ shiftOut(8-bit pattern)
       │
       └─→ display.update(displayState, value)
               │
               └─→ lc.setDigit() / lc.setChar()
```
...
main.cpp
  │
  ├─► config.h           (pin defs, constants, enums)
  ├─► state_manager.h    (state storage)
  ├─► mode_controller.h  (control logic)
  │     │
  │     ├─► switches.h         (input handling)
  │     ├─► relays.h           (output control)
  │     ├─► midi_handler.h     (MIDI output)
  │     └─► state_manager.h    (reads/writes state)
  │
  ├─► display.h          (7-segment)
  │     └─► LedControl   (external library)
  │
  ├─► led_controller.h   (shift register LEDs)
  ├─► relays.h          (relay control)
  ├─► switches.h        (switch reading)
  └─► midi_handler.h    (MIDI protocol)

External Dependencies:
  - Arduino.h (core framework)
  - EEPROM.h (AVR EEPROM library)
  - LedControl.h (MAX7219 driver - PlatformIO lib)
```
...
```cpp
Bank Mode Preset Number:
    
    presetNumber = (currentBank × 4) + switchIndex + 1
    
    Examples:
      Bank 1, SW1 (index 0): (1 × 4) + 0 + 1 = 5... wait, that's wrong!
      
    Actually:
      Bank 0, SW1: PC 1  = ((0-1) × 4) + 0 + 1 = ... no
      
    Looking at code in mode_controller.cpp line 190:
      uint8_t pc = (state.currentBank * 4) + switchIndex + 1;
      
    Correction (banks are 1-32 in display):
      Bank 1, SW1: (1 × 4) + 0 + 1 = 5  
      Bank 0, SW1: (0 × 4) + 0 + 1 = 1  ← currentBank must start at 0!
      
    But state_manager.cpp shows: currentBank(1)
    
    So actual formula with displayed banks (1-32):
      Bank 1 (displayed), SW1: (1 × 4) + 0 + 1 = 5  
      Bank 0 (displayed as 1), SW1: (0 × 4) + 0 + 1 = 1
      
    Wait, let me check the code again...
    
    Ah! Current implementation:
      currentBank = 1 to 32 (as displayed)
      PC range: 5-132?  
      
    This needs verification - might be a bug!
    Should be: PC 1-4 (Bank 1), 5-8 (Bank 2), ..., 125-128 (Bank 32)
    
    With currentBank 1-32:
      Bank 1, SW1: (1 × 4) + 0 + 1 = 5 ❌ Should be 1
      
    Fix needed:
      uint8_t pc = ((state.currentBank - 1) × 4) + switchIndex + 1;
```

**⚠️ FOUND BUG! Program Change calculation is off by 4!**
